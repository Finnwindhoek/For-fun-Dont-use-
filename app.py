...
def load_assets():
    ...
    try:
        ...
        # --- THIS IS THE FIX ---
        # 1. Load the dictionary of all models from the file using the plural key 'models'.
        all_models = assets['models']
        # 2. Select the specific model you want the app to use. Let's use SVM_linear.
        selected_model = all_models['SVM_linear']
        # 3. Return the selected model along with the other assets.
        return selected_model, assets['scaler'], assets['columns'], assets['imputation']
        # --------------------
        ...
        st.error("The asset file is corrupted or invalid. Please recreate it and ensure it contains 'models', 'scaler', 'columns', and 'imputation' keys.")
        return None, None, None, None
...
model, scaler, model_columns, imputation_values = load_assets()


# ----------------------------
# Prediction Function
# ----------------------------
def predict(data: pd.DataFrame, model, scaler, model_cols, imputation_vals):
    """
    ...
    # --- ADD THIS NEW BLOCK AT THE TOP OF THE FUNCTION ---
    # Step 0: Fill missing values using the saved medians and modes.
    df.fillna(imputation_vals['medians'], inplace=True)
    df.fillna(imputation_vals['modes'], inplace=True)
    # --------------------------------------------------------
...
if model is None or scaler is None or model_columns is None:
    ...
    with tab1:
        ...
        if st.button("🚀 Predict Sales Class", type="primary", use_container_width=True):
            ...
            prediction = predict(input_df, model, scaler, model_columns, imputation_values)
                ...
                if st.button("🔮 Run Batch Prediction", type="primary", use_container_width=True):
                    with st.spinner('Processing your file... This may take a moment.'):
                        ...
                        if not all(col in batch_df.columns for col in required_cols):
                            ...
                            predictions = predict(batch_df[required_cols], model, scaler, model_columns, imputation_values)
